...
export const POST = protectedRoute(async (request: NextRequest, { user }) => {
    ...
    await prisma.$transaction(async (tx: any) => {
        await tx.item.update({ where: { id: item_id }, data: { stockQuantity: { decrement: quantity }, damageQuantity: { increment: quantity } } });
        await tx.stockMovement.create({
            data: { itemId: item_id, movementType: 'damage', quantity, referenceType: 'manual', notes: notes || 'Damage recorded', movementDate: new Date(), createdBy: user.user_id },
        });
    });

    return Response.json({ success: true, message: 'Damage recorded successfully' });
}, { requiredPermission: 'manage:inventory', auditAction: 'DAMAGE_RECORD', auditEntity: 'inventory' });
